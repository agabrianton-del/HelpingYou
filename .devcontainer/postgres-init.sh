...
database_exists() {
  printf "SELECT 1 FROM pg_database WHERE datname = :'test_db_name'\n" |
    PGPASSWORD="${PGPASSWORD:-postgres}" psql -h 127.0.0.1 -U "$db_user" -d "$db_name" -v test_db_name="$test_db_name" -tA |
    grep -q 1
}

until PGPASSWORD="${PGPASSWORD:-postgres}" psql -h 127.0.0.1 -U "$db_user" -d "$db_name" -c "SELECT 1" >/dev/null 2>&1; do
  sleep 1
done
...
if ! database_exists; then
  ...
  printf "SELECT format('CREATE DATABASE %%I', :'test_db_name') \\\\gexec\n" |
    PGPASSWORD="${PGPASSWORD:-postgres}" psql -h 127.0.0.1 -U "$db_user" -d "$db_name" -v test_db_name="$test_db_name" ||
    create_status=$?
